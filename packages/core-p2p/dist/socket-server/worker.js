...
class Worker extends scworker_1.default {
    ...
    async run() {
        ...
        this.scServer.setCodecEngine(sc_codec_1.codec);
        await this.loadRateLimitedEndpoints();
        await this.loadConfiguration();
        this.rateLimiter = utils_1.buildRateLimiter({
        ...
        this.scServer.wsServer.on("connection", (ws, req) => {
            ...
            const existingSockets = clients.filter(client => client.remoteAddress === req.socket.remoteAddress && client.remotePort !== req.socket.remotePort);
    ...
    async loadRateLimitedEndpoints() {
    ...
    hasAdditionalProperties(object) {
        ...
        if (object.data.data) {
            ...
            const schema = schemas_1.requestSchemas[version][handler];
            try {
                if (object.event === "p2p.peer.postTransactions") {
                    if (typeof object.data.data === "object" &&
                        ...
                        for (const transaction of object.data.data.transactions) {
                            if (!validate_1.validateTransactionLight(transaction)) {
                                return true;
                            }
                        }
                    }
                    ...
                    if (!(typeof object.data.data === "object" &&
                        ...
                        Object.keys(object.data.data.block).length === 2) // {base64, data}
                    ) {
                        return true;
                    }
                }
        ...
        if (object.data.headers) {
            ...
            if ((object.data.headers.version && typeof object.data.headers.version !== "string") ||
    ...
    setErrorForIpAndDestroy(socket) {
    ...
    async handleSocket(socket) {
        ...
        const cidrRemoteAddress = ip_1.cidr(`${ip}/24`);
        const sameSubnetSockets = Object.values({ ...this.scServer.clients, ...this.scServer.pendingClients }).filter(client => ip_1.cidr(`${client.remoteAddress}/24`) === cidrRemoteAddress);
        if (sameSubnetSockets.length > this.config.maxSameSubnetPeers) {
            socket.destroy();
            return;
        }
    }
    async handleEmit(req, next) {
        if (req.event.length > 128) {
            req.socket.terminate();
            return;
        }
        const rateLimitedEndpoints = this.getRateLimitedEndpoints();
        const useLocalRateLimiter = !rateLimitedEndpoints[req.event];
        ...
        try {
            ...
            const isAppReady = (await this.sendToMasterAsync("p2p.utils.isAppReady")).data.ready;
            if (!isAppReady) {
            ...
            if (version === "internal") {
                ...
                const requestSchema = schemas_1.requestSchemas.peer[handler];
    ...
    async sendToMasterAsync(endpoint, data) {
