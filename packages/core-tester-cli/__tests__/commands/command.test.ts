...
describe("Command Base", () => {
  describe("Init", () => {
    ...
    it("initialize with option", async () => {
      mockAxios.onGet("http://test_baseUrl:1234/api/v2/node/configuration").reply(200, { data: { constants: {} } });
  ...
  describe("getVoters", () => {
    ...
    it("should get voters", async () => {
      ...
      expect(await command.getVoters(1)).toIncludeSameMembers([...voterPage1Fixture.data, ...voterPage2Fixture.data]);
    });
  });
  ...
  describe("__loadConstants", () => {
    ...
    it("should sets constant", async () => {
      ...
      expect(command.config.constants).toContainAllEntries([["testConstant", true as any], ["testConstant2", "test"]]);
    });
  });
