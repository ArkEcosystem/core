...
export class ModelConverter implements Contracts.Database.ModelConverter {
    ...
    public getBlockModels(blocks: BlockInterfaces.IBlock[]): Contracts.Database.BlockModel[] {
        return blocks.map((b) => Object.assign(new Block(), b.data));
    }

    public getBlockData(models: Contracts.Database.BlockModel[]): BlockInterfaces.IBlockData[] {
        return models;
    }
    ...
    public getTransactionDataWithBlockData(
        ...
        return transactionData.map((data) => {
            ...
            AppUtils.assert.defined<BlockInterfaces.IBlockData>(block);
            return { data, block };
        });
    }
}
