        ...
        if (orderBy && typeof orderBy === "string") {
            ...
            if (fieldDirection.length === 2 && (fieldDirection[1] === "asc" || fieldDirection[1] === "desc")) {
                searchParameters.orderBy.push({
                    field: lodash_snakecase_1.default(fieldDirection[0]),
        ...
        for (const fieldName of fieldNames) {
            ...
            if (!fieldDescriptor) {
                searchParameters.parameters.push({
                    field: fieldName,
                    operator: core_interfaces_1.Database.SearchOperator.OP_CUSTOM,
                    value: params[fieldName],
                });
                continue;
            }
            if (fieldDescriptor.supportedOperators.includes(core_interfaces_1.Database.SearchOperator.OP_LIKE)) {
                searchParameters.parameters.push({
                    field: fieldName,
                    operator: core_interfaces_1.Database.SearchOperator.OP_LIKE,
                    value: `%${params[fieldName]}%`,
                });
                continue;
            }
            // 'between'
            if (fieldDescriptor.supportedOperators.includes(core_interfaces_1.Database.SearchOperator.OP_GTE) ||
                fieldDescriptor.supportedOperators.includes(core_interfaces_1.Database.SearchOperator.OP_LTE)) {
                ...
                if (!params[fieldName].hasOwnProperty("from") && !params[fieldName].hasOwnProperty("to")) {
                    ...
                    if (params[fieldName].hasOwnProperty("from")) {
                        searchParameters.parameters.push({
                            field: fieldName,
                            operator: core_interfaces_1.Database.SearchOperator.OP_GTE,
                    ...
                    if (params[fieldName].hasOwnProperty("to")) {
                        searchParameters.parameters.push({
                            field: fieldName,
                            operator: core_interfaces_1.Database.SearchOperator.OP_LTE,
            ...
            if (fieldDescriptor.supportedOperators.includes(core_interfaces_1.Database.SearchOperator.OP_IN) &&
                Array.isArray(params[fieldName])) {
                searchParameters.parameters.push({
                    field: fieldName,
                    operator: core_interfaces_1.Database.SearchOperator.OP_IN,
                    value: params[fieldName],
                });
                continue;
            }
            // if the field supports EQ, then ignore any others.
            if (fieldDescriptor.supportedOperators.includes(core_interfaces_1.Database.SearchOperator.OP_EQ)) {
                searchParameters.parameters.push({
                    field: fieldName,
                    operator: core_interfaces_1.Database.SearchOperator.OP_EQ,
                    value: params[fieldName],
            ...
            if (fieldDescriptor.supportedOperators.includes(core_interfaces_1.Database.SearchOperator.OP_CONTAINS)) {
                searchParameters.parameters.push({
                    field: fieldName,
                    operator: core_interfaces_1.Database.SearchOperator.OP_CONTAINS,
                    value: params[fieldName],
                });
                continue;
            }
