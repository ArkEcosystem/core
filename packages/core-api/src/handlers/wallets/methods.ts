...
export const registerMethods = (app: Contracts.Kernel.Application, server) => {
    ...
    const top = async request => {
        const wallets = app
            .get<Contracts.Database.DatabaseService>(Container.Identifiers.DatabaseService)
            .wallets.top(Contracts.Database.SearchScope.Wallets, server.app.utils.paginate(request));

        return server.app.utils.toPagination(wallets, "wallet");
    };

    const show = async request => server.app.utils.respondWithResource(findWallet(request.params.id), "wallet");
    ...
    const transactions = async request => {
        ...
        return server.app.utils.toPagination(rows, "transaction", (request.query.transform as unknown) as boolean);
    };
    ...
    const locks = async request => {
        ...
        // Sorry, cold wallets
        if (!wallet.publicKey) {
            return server.app.utils.toPagination({ rows: [], count: 0 }, "lock");
        }
        ...
        return server.app.utils.toPagination(rows, "lock");
    };
    ...
    app.resolve<ServerCache>(ServerCache)
        ...
        .method("v2.wallets.top", top, 30, request => server.app.utils.paginate(request))
