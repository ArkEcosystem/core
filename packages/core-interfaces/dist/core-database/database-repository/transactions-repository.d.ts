...
export interface ITransactionsRepository extends IRepository {
    ...
    findByBlockId(blockId: string): Promise<Array<{
        id: string;
        serialized: Buffer;
    }>>;
    latestByBlock(blockId: string): Promise<Array<{
        id: string;
        serialized: Buffer;
    }>>;
    latestByBlocks(blockIds: string[]): Promise<Array<{
        id: string;
        blockId: string;
        serialized: Buffer;
    }>>;
    ...
    getFeeStatistics(days: number, minFeeBroadcast: number): Promise<Array<{
        type: number;
        fee: number;
        timestamp: number;
    }>>;
