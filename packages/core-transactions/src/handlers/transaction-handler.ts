...
export abstract class TransactionHandler {
    ...
    public dynamicFee({
        ...
        satoshiPerByte,
        transaction,
    }: Contracts.Shared.DynamicFeeContext): Types.BigNumber {
        ...
        return this.cryptoManager.LibraryManager.Libraries.BigNumber.make(addonBytes + transactionSizeInBytes).times(
            satoshiPerByte,
        );
    }
    ...
    public async applyToSender(
        ...
        const newBalance: Types.BigNumber = sender.balance.minus(data.amount).minus(data.fee);

        assert(this.cryptoManager.LibraryManager.Utils.isException(transaction.data.id) || !newBalance.isNegative());
    ...
    protected async performGenericWalletChecks(
        ...
        if (sender.hasSecondSignature()) {
            ...
            const isException =
                ...
                this.cryptoManager.MilestoneManager.getMilestone().ignoreInvalidSecondSignatureField;
            if (!isException) {
                throw new UnexpectedSecondSignatureError();
            }
        }
        ...
        if (isMultiSignatureRegistration && !this.cryptoManager.MilestoneManager.getMilestone().aip11) {
    ...
    protected verifyTransactionNonceApply(wallet: Contracts.State.Wallet, transaction: Interfaces.ITransaction): void {
        ...
        const nonce: Types.BigNumber = transaction.data.nonce || AppUtils.BigNumber.ZERO;
    ...
    public abstract getConstructor(): Transactions.TransactionConstructor<BlockInterfaces.IBlockData>;
