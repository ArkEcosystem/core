...
export class Worker implements Contracts.TransactionPool.Worker {
    ...
    public async getTransactionFromData(
        ...
        const currentHeight = this.cryptoManager.HeightTracker.getHeight()!;
        if (currentHeight !== this.lastHeight) {
            this.lastHeight = currentHeight;
            this.ipcSubprocess.sendAction("setConfig", this.cryptoManager.NetworkConfigManager.all());
        ...
        return this.transactionsManager.TransactionFactory.fromBytesUnsafe(Buffer.from(serialized, "hex"), id);
    }
}
