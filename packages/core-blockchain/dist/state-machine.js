...
const blockchain_1 = require("./machines/blockchain");
...
const stateStorage = core_container_1.app.resolvePlugin("state").getStore();
/**
 * @type {IStateStore}
 */
blockchain_1.blockchainMachine.state = stateStorage;
/**
 * The blockchain actions.
 * @param  {Blockchain} blockchain
 * @return {Object}
 */
blockchain_1.blockchainMachine.actionMap = (blockchain) => ({
    blockchainReady: () => {
        if (!stateStorage.started) {
            stateStorage.started = true;
            emitter.emit(core_event_emitter_1.ApplicationEvents.StateStarted, true);
        }
    },
    checkLater() {
    ...
    exitApp() {
        core_container_1.app.forceExit("Failed to startup blockchain. Exiting ARK Core!");
    },
    async init() {
        try {
            const block = blockchain.state.getLastBlock();
    ...
    async downloadBlocks() {
        const lastDownloadedBlock = stateStorage.lastDownloadedBlock || stateStorage.getLastBlock().data;
        const blocks = await blockchain.p2p
            .getMonitor()
        ...
        const chained = !empty && (core_utils_1.isBlockChained(lastDownloadedBlock, blocks[0]) || crypto_1.Utils.isException(blocks[0]));
        if (chained) {
            logger.info(`Downloaded ${blocks.length} new ${pluralize_1.default("block", blocks.length)} accounting for a total of ${pluralize_1.default("transaction", blocks.reduce((sum, b) => sum + b.numberOfTransactions, 0), true)}`);
            try {
    ...
    async startForkRecovery() {
        ...
        const blocksToRemove = stateStorage.numberOfBlocksToRollback || random;
        await blockchain.removeBlocks(blocksToRemove);
        stateStorage.numberOfBlocksToRollback = undefined;
        logger.info(`Removed ${pluralize_1.default("block", blocksToRemove, true)}`);
    ...
    async rollbackDatabase() {
        ...
        const { maxBlockRewind, steps } = core_container_1.app.resolveOptions("blockchain").databaseRollback;
        ...
        logger.info(`Database integrity verified again after rollback to height ${lastBlock.data.height.toLocaleString()}`);
        blockchain.dispatch("SUCCESS");
    },
});
exports.stateMachine = blockchain_1.blockchainMachine;
