...
blockchainMachine.actionMap = blockchain => ({
  ...
  async init() {
    try {
      ...
      if (process.env.NODE_ENV === "test") {
        ...
        stateStorage.setLastBlock(new Block(config.genesisBlock));
        await blockchain.database.buildWallets(block.data.height);
      ...
      if (roundCalculator.isNewRound(block.data.height + 1)) {
        ...
        logger.info(`New round ${round.toLocaleString()} detected. Cleaning calculated data before restarting!`);
  ...
  async rebuildBlocks() {
    ...
    if (!blocks || blocks.length === 0) {
      ...
      logger.info(
        `Downloaded ${blocks.length} new ${pluralize("block", blocks.length)} accounting for a total of ${pluralize(
          "transaction",
