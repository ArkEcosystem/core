...
export class Deserializer<T, U extends ITransactionData, E> {
    ...
    public applyV1Compatibility(transaction: U): void {
        ...
        if (transaction.type === TransactionType.Vote && transaction.senderPublicKey) {
            transaction.recipientId = this.cryptoManager.Identities.Address.fromPublicKey(transaction.senderPublicKey);
        } else if (
    ...
    public deserialize(serialized: string | Buffer, options: IDeserializeOptions = {}): ITransaction<U, E> {
        const data = {} as U;
        ...
        if (data.version) {
            if (
                ...
                this.cryptoManager.LibraryManager.Utils.isSupportedTransactionVersion(data.version)
            ) {
                if (data.version === 1) {
    ...
    private deserializeCommon(transaction: U, buf: ByteBuffer): void {
        ...
        if (transaction.version === 1) {
            ...
            transaction.nonce = this.cryptoManager.LibraryManager.Libraries.BigNumber.make(buf.readUint64().toString());
        }

        transaction.senderPublicKey = buf.readBytes(33).toString("hex");
        transaction.fee = this.cryptoManager.LibraryManager.Libraries.BigNumber.make(buf.readUint64().toString());
        transaction.amount = this.cryptoManager.LibraryManager.Libraries.BigNumber.ZERO;
    }

    private deserializeVendorField(transaction: ITransaction<U, E>, buf: ByteBuffer): void {
    ...
    private deserializeSignatures(transaction: U, buf: ByteBuffer): void {
        if (transaction.version === 1) {
    ...
    private deserializeSchnorrOrECDSA(transaction: U, buf: ByteBuffer): void {
    ...
    private deserializeECDSA(transaction: U, buf: ByteBuffer): void {
    ...
    private deserializeSchnorr(transaction: U, buf: ByteBuffer): void {
