...
export class Serializer<T, U extends ITransactionData, E> {
    ...
    public getBytes(transaction: U, options: ISerializeOptions = {}): Buffer {
        ...
        if (
            ...
            this.cryptoManager.LibraryManager.Utils.isSupportedTransactionVersion(version)
        ) {
            if (version === 1) {
     ...
     */
    public serialize(transaction: ITransaction<U, E>, options: ISerializeOptions = {}): Buffer {
     ...
     */
    private getBytesV1(transaction: U, options: ISerializeOptions = {}): Buffer {
        let assetSize = 0;
        ...
        if (transaction.senderPublicKey) {
            ...
            if (isBrokenTransaction || (transaction.recipientId && transaction.type !== 1 && transaction.type !== 4)) {
                const recipientId =
                    ...
                    this.cryptoManager.Identities.Address.fromPublicKey(transaction.senderPublicKey);
                const recipient = this.cryptoManager.Identities.Address.toBuffer(recipientId).addressBuffer;
    ...
    private serializeCommon(transaction: ITransactionData, buffer: ByteBuffer): void {
        ...
        buffer.writeByte(transaction.network || this.cryptoManager.NetworkConfigManager.get("network.pubKeyHash"));
    ...
    private serializeVendorField(transaction: ITransaction<U, E>, buffer: ByteBuffer): void {
        if (transaction.hasVendorField()) {
            const { data }: ITransaction<U, E> = transaction;
    ...
    private serializeSignatures(
        ...
        if (transaction.signatures) {
            if (transaction.version === 1 && this.cryptoManager.LibraryManager.Utils.isException(transaction.id)) {
