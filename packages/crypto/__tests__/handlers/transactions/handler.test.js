...
beforeEach(() => {
  ...
  wallet = {
    ...
    balance: Bignum.from(4527654310),
...
describe('Handler', () => {
  ...
  describe('applyTransactionToSender', () => {
    ...
    it('should be ok', () => {
      ...
      wallet.balance = Bignum.from(initialBalance)

      handler.applyTransactionToSender(wallet, transaction)

      expect(wallet.balance).toEqual(Bignum.from(initialBalance).subtract(transaction.amount).subtract(transaction.fee))
    })
    ...
    it('should not be ok', () => {
      ...
      expect(wallet.balance).toEqual(Bignum.from(initialBalance))
    })
  })
  ...
  describe('revertTransactionForSender', () => {
    ...
    it('should be ok', () => {
      ...
      expect(wallet.balance).toEqual(Bignum.from(initialBalance).add(transaction.amount).add(transaction.fee))
    })
  ...
  describe('applyTransactionToRecipient', () => {
    ...
    it('should be ok', () => {
      ...
      expect(wallet.balance).toEqual(Bignum.from(initialBalance).add(transaction.amount))
    })
  ...
  describe('revertTransactionForRecipient', () => {
    ...
    it('should be ok', () => {
      ...
      expect(wallet.balance).toEqual(Bignum.from(initialBalance - transaction.amount))
    })
