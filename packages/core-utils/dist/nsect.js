...
class NSect {
    ...
    async find(low, high) {
        ...
        for (;;) {
            ...
            const temp = await this.probe(indexesToProbe);
            if (temp === undefined) {
                break;
            }
            ...
            const indexOfHighestMatching = indexesToProbe.indexOf(highestMatching);
     ...
     */
    calcProbes(low, high) {
        assert_1.default(low <= high, `${low} <= ${high}`);
        const diff = high - low;
        const p = new Set();
        for (let i = 0; i < this.nAry + 1; i++) {
            const h = low + Math.round((diff * i) / this.nAry);
            p.add(h);
        }
        return Array.from(p);
    }
