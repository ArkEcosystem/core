...
/**
 * At the moment there is no way to extend the crypto package to add your own transaction builders directly onto an existing instance which results in an odd DX as you have to access some builders from the official crypto package and some from another.

For this purpose, an extend method will be added which will allow the crypto package to register new transaction builders and then access them via crypto.getTransactionBuilder('Transfer'). This ensures that all crypto package related actions are going through the crypto package and aren't scattered out through 5 packages from bridgechains.
 */
