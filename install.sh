...
if [[ "$choice" =~ ^(yes|y|Y) ]]; then
    ...
    radians env:set --key=CORE_DB_USERNAME --value="${databaseUsername}"
    radians env:set --key=CORE_DB_PASSWORD --value="${databasePassword}"
    radians env:set --key=CORE_DB_DATABASE --value="${databaseName}"
