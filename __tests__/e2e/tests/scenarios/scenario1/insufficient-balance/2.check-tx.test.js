...
describe("Check that only no transaction was accepted", () => {
    it("should have no transaction accepted", async () => {
        ...
        const txTransfers = response.data.data.filter(
            transaction => transaction.recipient === utils.transferRecipient.address,
        );
        ...
        const txTransfers2ndSig = response.data.data.filter(
            transaction => transaction.recipient === utils.transfer2ndsigRecipient.address,
        );
        ...
        const tx2ndSigReg = response.data.data.filter(
            transaction => transaction.sender === utils.secondsigRegSender.address,
        );
