...
describe('Check that only 1 transaction out of the 2 was accepted', () => {
  it('should have 1 transaction accepted', async () => {
    ...
    const txTransfers = response.data.data.filter(transaction => transaction.recipient === utils.doubleTransferRecipient.address)
    expect(txTransfers.length).toBe(1) // 1 transaction was sent to this address

    // double spend - transfer with 2nd signature
    const txTransfers2ndSig = response.data.data.filter(transaction => transaction.recipient === utils.doubleTransfer2ndsigRecipient.address)
    ...
    const txDelReg = response.data.data.filter(transaction => transaction.sender === utils.doubleDelRegSender.address)
    expect(txDelReg.length).toBe(1) // 1 transaction was sent from this address

    // double spend - 2nd signature registration
    const tx2ndSigReg = response.data.data.filter(transaction => transaction.sender === utils.double2ndsigRegSender.address)
