...
import { htlcSecretHex } from "../crypto/transactions/__fixtures__/htlc";

let transaction;
let transactionSchema: TransactionSchema;
let TransactionTypeFactory;
...
beforeAll(() => {
    ...
    Ajv = crypto.Validator;
    BuilderFactory = crypto.TransactionManager.BuilderFactory;
    TransactionTypeFactory = crypto.TransactionManager.TransactionTools.TransactionTypeFactory;
});
...
describe("Transfer Transaction", () => {
    ...
    it("should be valid with correct data", () => {
        transaction
            .recipientId(address)
            .amount(amount)
            .fee(crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(fee).toFixed())
            .vendorField("Ahoy")
            .sign("passphrase");
    ...
    it("should be ok and turn uppercase publicKey to lowercase", () => {
        const transfer = transaction.recipientId(address).amount("1").fee("1").sign("passphrase").build();
...
describe("Multi Signature Registration Transaction", () => {
    ...
    beforeAll(() => {
        ...
        Ajv = crypto.Validator;
        BuilderFactory = crypto.TransactionManager.BuilderFactory;
        PublicKey = crypto.CryptoManager.Identities.PublicKey;
        TransactionTypeFactory = crypto.TransactionManager.TransactionTools.TransactionTypeFactory;
    });
...
describe("Multi Payment Transaction", () => {
    ...
    beforeAll(() => {
        ...
        multiPayment = BuilderFactory.multiPayment();
    });
    ...
    it("should not accept more than `multiPaymentLimit` payments", () => {
        const limit = crypto.CryptoManager.MilestoneManager.getMilestone().multiPaymentLimit;

        for (let i = 0; i < limit; i++) {
            multiPayment.addPayment(address, `${i + 1}`);
        }

        multiPayment.data.asset.payments.push({
            amount: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.ONE,
            recipientId: address,
        });
        multiPayment.sign("passphrase");
        ...
        crypto.CryptoManager.MilestoneManager.getMilestone().multiPaymentLimit = 10;
        ...
        crypto.CryptoManager.MilestoneManager.getMilestone().multiPaymentLimit = 2;
        expect(Ajv.validate(transactionSchema.$id, multiPayment.data).error).not.toBeUndefined();

        multiPayment.data.asset.payments = [
            { amount: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.ONE, recipientId: address },
            { amount: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.ONE, recipientId: address },
        ];

        expect(Ajv.validate(transactionSchema.$id, multiPayment.data).error).toBeUndefined();
        crypto.CryptoManager.MilestoneManager.getMilestone().multiPaymentLimit = limit;
        expect(Ajv.validate(transactionSchema.$id, multiPayment.data).error).toBeUndefined();
    });
