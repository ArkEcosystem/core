...
describe("Block", () => {
    ...
    describe("constructor", () => {
        ...
        it("should fail to verify a block with too large payload", () => {
            jest.spyOn(crypto.CryptoManager.MilestoneManager, "getMilestone").mockImplementation((height) => ({
                block: {
                    version: 0,
                    maxTransactions: 200,
            ...
            const transactions = TransactionFactory.initialize(crypto.CryptoManager)
                ...
                .withTimestamp(
                    optionsDefault.timestamp +
                        3600 +
                        crypto.CryptoManager.MilestoneManager.getMilestone(optionsDefault.previousBlock.height)
                            .blocktime,
                )
                .withPassphrase("super cool passphrase")
                .create();
        ...
        it("should fail to verify a block if error is thrown", () => {
            ...
            jest.spyOn(crypto.CryptoManager.LibraryManager.Crypto.Slots, "getSlotNumber").mockImplementation(
                (height) => {
                    throw errorMessage;
                },
            );
                ...
                if (key !== "transactions") {
                    if (bignumProperties.includes(key)) {
                        expect(header[key]).toEqual(
                            crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(data2[key]),
                        );
                    } else {
    ...
    it("should reorder correctly transactions in deserialization", () => {
        ...
        const issue = {
            ...
            totalFee: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(600000000),
            reward: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(200000000),
            payloadLength: 64,
            ...
            transactions: [
                {
                    ...
                    type: 1,
                    timestamp: 25028279,
                    fee: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(500000000),
                ...
                },
                {
                    ...
                    timestamp: 25028325,
                    senderPublicKey: "02aadc3e0993c1d3447db27741745eb9c2c6522cccf02fc8efe3bf2d49708243dd",
                    fee: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(100000000),
                    amount: crypto.CryptoManager.LibraryManager.Libraries.BigNumber.make(0),
                    asset: {
