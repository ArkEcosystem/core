...
const waitForEvent = (worker: Worker, message?: any): Promise<void> => {
    return new Promise<void>((resolve) => {
        ...
        worker.once("message", (data) => { onEvent("message", data) });
        worker.once("exit", (data) => { onEvent("exit", data) });
        worker.once("error", (data) => { onEvent("error", data) });
...
describe("Worker", () => {
    ...
    it("should catch unhandled rejection and pass it in message", async () => {
        ...
        expect(spyOnMessage).toHaveBeenCalledWith(expect.objectContaining({
            action: "exception"
        }));
    });
    ...
    it("should throw exception", async () => {
        ...
        await new Promise(resolve => {
            ...
            worker.once("message", (data) => {resolve()})
        })
