...
describe("ExpirationService.getTransactionExpirationHeight", () => {
    it("should return undefined when checking v2 transaction without expiration field", () => {
        const transaction = { data: { version: 2 } } as Interfaces.ITransaction;
        const expirationService = container.resolve(ExpirationService);
        const expirationHeight = expirationService.getTransactionExpirationHeight(transaction);

        expect(expirationHeight).toBeUndefined();
    });
...
describe("ExpirationService.isTransactionExpired", () => {
    it("should always return false when checking v2 transaction without expiration field", () => {
        const transaction = { data: { version: 2 } } as Interfaces.ITransaction;
        const expirationService = container.resolve(ExpirationService);
        const expired = expirationService.isTransactionExpired(transaction);
