...
class Transaction {
  ...
  constructor(transaction) {
    ...
    if(this.data.version == 1){
      if(transaction.amount > this.data.amount){
  ...
  serialize(transaction) {
    ...
    if(transaction.vendorField){
      let vf = new Buffer(transaction.vendorField, 'utf8');
    ...
    case 0: // Transfer
      ...
      bb.writeInt(transaction.expiration || 0);
    ...
    case 4: // Multi-Signature
      var keysgroupBuffer = new Buffer(transaction.asset.multisignature.keysgroup.map((k)=>{return k.slice(1);}).join(''), 'hex');
